...
# `setup do` syntaxes without pulling in ActiveSupport. Keeps the gem free of
# AS even in its test suite.
class ApiSerializerTestCase < Minitest::Test
  class << self
    def test(name, &block)
  ...
  # AS-compatible helper that Minitest doesn't provide out of the box.
